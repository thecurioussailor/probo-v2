...
export class SubscriptionManager {
    ...
    public unsubscribe(userId: string, subscription: string){
        ...
        if(userSubscriptions){
            this.subscriptions.set(userId, userSubscriptions.filter(s => s !== subscription))
        }

        const channelSubscribers = this.reverseSubscriptions.get(subscription);
        if(channelSubscribers) {
            this.reverseSubscriptions.set(subscription, channelSubscribers.filter(s => s !== userId));
